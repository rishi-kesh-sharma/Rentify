...
function* loadOne(action) {
  ...
  yield call(
    Api.get(
      `rent/sub_category/${action.payload}`,
...
function* redirectOnSuccess() {
  ...
  yield put(push('/admin/rent-sub-cat-manage'));
}
...
function* deleteSuccessFunc(action) {
  const snackbarData = {
    message: action.payload.msg || 'Rent Sub Category delete success!!',
    options: {
      variant: 'success',
    },
  };
...
function* getCount(action) {
  ...
  yield call(
    Api.get(
      `rent/count/sub_category/${action.payload}`,
