...
import {
  ...
  makeSelectQuery,
  makeSelectTag,
  makeSelectUpdateCalled,
  makeSelectUsers,
} from './selectors';

const key = 'rentManagePage';

/* eslint-disable react/prefer-stateless-function */
const RentManagePage = (props) => {
  ...
  const handleAdd = () => {
    props.clearOne();
    navigate('/admin/rent-manage/add');
  };

  const handleEdit = (id) => {
    navigate(`/admin/rent-manage/edit/${id}`);
  };

  const handleView = (slug_url) => {
    navigate(`/rent/${slug_url}`);
  };
  ...
  const tableData = data.map(
    ({
      ...
      <>
        <Link
          to={`/rent/${moment(added_at).format('YYYY/MM/DD')}/${_id}`}
          target="_blank"
      ...
      (sub_category && sub_category?.title) || 'No',
      ...
      (
        <p className="">
          {(added_by?.name && added_by?.name) || posted_by?.name}
        </p>
      ) || '',
    ],
  );
  ...
  return (
    <>
      <DeleteDialog
        open={state.open}
        doClose={handleClose}
        doDelete={() => handleDeleteRent(state.deleteId)}
      />
      <Helmet>
        <title>Rents</title>
      </Helmet>
      ...
      <PageContent loading={loading}>
        <TableFilter col={7}>
          ...
          {/* <SelectField
            ...
            onChange={handleQueryDropDownChange('posted_by')}
            options={listAuthor}
          /> */}
          <SelectField
            label="Showcase"
...
const mapStateToProps = createStructuredSelector({
  ...
  updateCalled: makeSelectUpdateCalled(),
});

export default connect(mapStateToProps, mapDispatchToProps)(RentManagePage);
